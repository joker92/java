 ...
 * @author svilupposw
 */
public class LapideDAO {
    public static void salva(Lapide c) {
        EntityManager em
                = PM.getEM();
    ...
    public static List<Lapide> elenco() {
    ...
    public static void elimina(Lapide c) {
        EntityManager em = PM.getEM();
        EntityTransaction et = em.getTransaction();
        try {
            et.begin();
            c = em.find(Lapide.class, c.getId());
            em.remove(c);
            et.commit();
        } finally {
            if (et.isActive()) {
