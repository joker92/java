 ...
 * @author svilupposw
 */
public class FuneraleDAO {
    public static void salva(Funerale c) {
        EntityManager em
                = PM.getEM();
    ...
    public static List<Funerale> elenco() {
    ...
    public static void elimina(Funerale c) {
        EntityManager em = PM.getEM();
        EntityTransaction et = em.getTransaction();
        try {
            et.begin();
            c = em.find(Funerale.class, c.getId());
            em.remove(c);
            et.commit();
        } finally {
            if (et.isActive()) {
